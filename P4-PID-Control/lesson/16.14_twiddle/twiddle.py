...
def twiddle(tol=0.2): 
    ...
    dp = [1, 1, 1]
    robot = make_robot()
    x_trajectory, y_trajectory, best_err = run(robot, pamameters)
    
    # TODO: twiddle loop here
    it = 0
    while sum(dp) > tol:
        print("Iteration {}, best error = {}".format(it, best_err))
        for i in range(len(pamameters)):
            pamameters[i] += dp[i]
            robot = make_robot()
            x_trajectory, y_trajectory, err = run(robot, pamameters)
